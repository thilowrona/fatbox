...
def distance_between_nodes_cor(G, n0, n1):
    x0 = G.nodes[n0]['x']
    y0 = G.nodes[n0]['y']
    x1 = G.nodes[n1]['x']
    y1 = G.nodes[n1]['y']     
...
def compute_edge_length(G):
    for edge in G.edges:
        G.edges[edge]['length'] = distance_between_nodes_cor(G, edge[0], edge[1])
    return G
...
def fault_lengths(G):    
    lengths = np.zeros(nx.number_connected_components(G))
    for n, c in enumerate(sorted(nx.connected_components(G))):        
        G_sub = G.subgraph(c)        
    ...
    for node in H.nodes:
        if node[1] == 0:
            ...
            if H.nodes[(node[0], 1)]['v_x'] == 0 or H.nodes[(node[0], 2)]['v_x'] == 0:
                G.nodes[node[0]]['heave'] =  0
                G.nodes[node[0]]['throw'] =  0
                G.nodes[node[0]]['displacement']    =  0
            else:            
                G.nodes[node[0]]['heave'] =  abs((H.nodes[(node[0], 1)]['v_x'] - H.nodes[(node[0], 2)]['v_x']))*dt
                G.nodes[node[0]]['throw'] =  abs((H.nodes[(node[0], 1)]['v_z'] - H.nodes[(node[0], 2)]['v_z']))*dt
